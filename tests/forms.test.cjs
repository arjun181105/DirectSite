...
async function scenario(mode,kind){
  ...
  await window.__step2(kind);
  if(mode==='success'){assert(events.includes('form_submit'));assert(events.includes('demo_requested'));assert.equal(status.textContent,'');await window.__step2(kind);assert.equal(events.filter(x=>x==='form_submit').length,1);}
...
(async()=>{for(const kind of ['new','redesign'])for(const mode of ['success','http-failure','api-failure','network','bad-json','invalid'])await scenario(mode,kind);console.log('PASS: both demo branches handle acceptance, rejection and retries without duplicate conversions.');})().catch(e=>{console.error(e);process.exitCode=1;});
